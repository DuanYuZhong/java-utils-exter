...
public class StringUtil extends org.apache.commons.lang3.StringUtils{
    ...
    /**
     * 处理String类型的persons和departments，输出数组集合
     * @param persons 选择的人
     * @param departments 选择的部门
     * @return
     */
    public List<Integer[]> toIntegerArr(String persons, String departments){
        List<Integer[]> list = new ArrayList<Integer[]>();
        Integer[] visibleDepartments=null;
        Integer[] visiblePersons=null;
        if(!(persons==null||"".equals(persons))){
            String[] vPersons = persons.split(",");
            visiblePersons = new Integer[vPersons.length];
            for(int i=0;i<vPersons.length;i++){
                visiblePersons[i] = Integer.parseInt(vPersons[i]);
            }
        }
        if(!(departments==null||"".equals(departments))){
            String[] vDepartments = departments.split(",");
            visibleDepartments = new Integer[vDepartments.length];
            for(int i =0;i<vDepartments.length;i++){
                if(vDepartments[i]!=null&&!"".equals(vDepartments[i])){
                    visibleDepartments[i]=Integer.parseInt(vDepartments[i]);
                }
            }
        }
        list.add(visiblePersons);
        list.add(visibleDepartments);
        return list;
    }
